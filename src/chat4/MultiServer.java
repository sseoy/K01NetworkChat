...
public class MultiServer {
	static ServerSocket  serverSoket=null;//클라이언트의 요청을 받기 위해 준비
	static Socket socket = null;//서버에 접속 요청을 한다.
	...
	public static void init() {
		...
		try {
			...
			//accept 클라이언트의 접속요청을 허가함
			socket = serverSoket.accept();
			System.out.println(socket.getInetAddress()+":"+ 
					socket.getPort());
			
			//클라이언트로 메세지를 보낸 준비(output 스트림)
			out = new PrintWriter(socket.getOutputStream(), true);
			//클라이언트로 메세지를 보낼 준비(input스트림)
